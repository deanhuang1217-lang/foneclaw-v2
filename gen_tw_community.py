#!/usr/bin/env python3
"""Generate Traditional Chinese (Taiwan) community page from source-backed i18n."""
...
COMMUNITY_LINKS = [
    ('Telegram', '產品更新與 beta 訊息', '加入', 'https://t.me/foneclaw'),
    ('Discord', '即時交流與開發討論', '加入', 'https://discord.gg/vQ65DUHucj'),
    ('X', '最新消息與產品動態', '追蹤', 'https://x.com/huangda_huang'),
    ('Facebook', '社群更新與討論', '追蹤', 'https://www.facebook.com/foneclaw'),
    ('YouTube', '影片教學與產品展示', '訂閱', 'http://www.youtube.com/@foneclaw'),
    ('Instagram', '圖片、短片與幕後內容', '追蹤', 'https://www.instagram.com/foneclaw/'),
    ('Reddit', '使用者討論與回饋', '加入', 'https://www.reddit.com/user/foneclaw/'),
    ('Feedback', '功能建議與問題回報', '寄信', 'mailto:Feedback@foneclaw.ai'),
]

def generate():
    title = get_text('community', 'meta_title', 'tw')
    desc = get_text('community', 'meta_desc', 'tw')
    hero_title = get_text('community', 'hero_title', 'tw')
    hero_desc = get_text('community', 'hero_desc', 'tw')
    extra_css = '''<style>
...
<section class="community-section"><div class="wrap"><div class="community-grid">{''.join(cards)}</div></div></section>'''
    html=[]
    html.append(head(title, desc, '/tw/community.html', extra_css=extra_css, og_image=f'{BASE}/images/home_arch.jpg', lang='tw', hreflang_tags=generate_hreflang_tags('/community.html','tw')))
    html.append(nav(4, 'tw'))
    html.append('<main>')
    html.append(body)
    html.append('</main>')
    html.append(footer('tw'))
